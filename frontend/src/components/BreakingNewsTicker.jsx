...
export default function BreakingNewsTicker() {
  ...
  return (
    <div className="w-full bg-white border-b border-gray-200 flex items-center overflow-hidden relative z-40 h-10 shadow-sm">
      {/* Label Box */}
          ...
          {newsItems.map((item, idx) => (
            <React.Fragment key={item.id}>
              <Link 
                ...
                className="text-gray-700 hover:text-[#E50914] hover:underline text-sm font-medium transition-colors"
              >
                {item.title}
              </Link>
