...
export default function Contact() {
  ...
  return (
    <div className="min-h-screen bg-[#0B0F14] text-white flex flex-col items-center justify-center pt-28 px-4 w-full">
      <div className="max-w-4xl w-full text-center">
        ...
        <h1 className="text-4xl md:text-5xl font-extrabold mb-4 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          যোগাযোগ{' '}
        ...
        <p className="text-[#94A3B8] mb-12 max-w-2xl mx-auto text-base md:text-lg leading-relaxed animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
          আপনার যেকোনো অভিযোগ, তথ্য বা পরামর্শ আমাদের জানাতে নিচের প্ল্যাটফর্মগুলোতে যুক্ত হোন। আমরা আপনাদের পাশে আছি।
        </p>

        <div className="flex flex-wrap justify-center gap-3 md:flex-nowrap md:gap-5 stagger-children">
          {platforms.map((platform, idx) => (
            <a
              ...
              className={`w-[calc(50%-6px)] md:flex-1 group flex flex-col items-center justify-center p-5 md:p-8 glass-card rounded-2xl transition-all duration-300 transform hover:-translate-y-2 hover:shadow-2xl ${platform.hoverColor} relative overflow-hidden`}
            >
              {/* Glow on hover */}
              <div 
                className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl"
                style={{ background: `radial-gradient(circle at 50% 40%, ${platform.glowColor}15, transparent 70%)` }}
              />
              
              <div className="transform group-hover:scale-110 transition-transform duration-300 relative z-10">
                {platform.icon}
              </div>
              <h3 className="text-sm md:text-base font-semibold text-[#94A3B8] group-hover:text-white transition-colors relative z-10">
                {platform.name}
              </h3>
            </a>
          ))}
